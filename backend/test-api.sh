...
# Test 2: Analyse Transcript
echo -e "${BLUE}Test 2: Analyse Transcript${NC}"
Analyse_RESPONSE=$(curl -s -X POST "${API_URL}/api/transcripts/Analyse" \
...
if echo "$Analyse_RESPONSE" | grep -q "id"; then
    echo -e "${GREEN}✓ Transcript analysis succeeded${NC}"
    ANALYSIS_ID=$(echo "$Analyse_RESPONSE" | jq -r '.id')
    echo "Analysis ID: $ANALYSIS_ID"
    echo "$Analyse_RESPONSE" | jq '.'
else
    echo -e "${RED}✗ Transcript analysis failed${NC}"
    echo "$Analyse_RESPONSE"
    exit 1
fi
echo ""
...
ERROR_RESPONSE=$(curl -s -X POST "${API_URL}/api/transcripts/Analyse" \
  -H "Content-Type: application/json" \
  -d '{"transcript": ""}')
...
LONG_RESPONSE=$(curl -s -X POST "${API_URL}/api/transcripts/Analyse" \
  -H "Content-Type: application/json" \
