...
export function AnalysisResults({ analysis, onNewAnalysis }: AnalysisResultsProps) {
  const getSentimentColor = (sentiment: string): 'success' | 'info' | 'warning' | 'danger' => {
    const lower = sentiment.toLowerCase();
    if (lower.includes('positive') || lower.includes('productive') || lower.includes('constructive')) {
      return 'success';
    }
    if (lower.includes('negative') || lower.includes('tense') || lower.includes('conflict')) {
      return 'danger';
    }
  ...
  const getPriorityColor = (priority: string | null): 'danger' | 'warning' | 'info' | undefined => {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              <h3 className="text-lg font-semibold text-gray-800">Meeting Sentiment</h3>
              <Tag
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold text-gray-800">
              Action Items {analysis.actionItems.length > 0 && `(${analysis.actionItems.length})`}
            </h3>
          </div>
              ...
              {analysis.actionItems.map((item) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    {item.priority && (
                      <i
                        className={`pi ${getPriorityIcon(item.priority)} text-lg mt-0.5`}
                        style={{
                          color:
                            item.priority === 'high'
                              ? '#ef4444'
      ...
      <Card>
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold text-gray-800">
              Decisions {analysis.decisions.length > 0 && `(${analysis.decisions.length})`}
            </h3>
          </div>
              ...
              {analysis.decisions.map((decision) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    <i
                      className={`pi ${
                        decision.type === 'made' ? 'pi-check-circle' : 'pi-clock'
                      } text-lg mt-0.5`}
                      style={{
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between gap-3 mb-2">
                        ...
                        <Tag
                          value={decision.type === 'made' ? 'DECIDED' : 'PENDING'}
                          severity={decision.type === 'made' ? 'success' : 'warning'}
                          rounded
                        />
                      </div>
                      {decision.context && (
      ...
      <div className="text-sm text-gray-500 text-center">
        Analysis ID: {analysis.id} • Generated: {new Date(analysis.createdAt).toLocaleString()}
      </div>
    </div>
  );
}
